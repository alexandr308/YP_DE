...
def upload_data_to_staging_inc(filename, date, pg_table, pg_schema, ti):
    ...
    if filename in ['user_orders_log_inc.csv']:
...
with DAG(
    ...
    # Загружаем файлы данных инкремента
    upload_user_order_log_inc = PythonOperator(
        task_id='upload_user_order_log_inc',
        python_callable=upload_data_to_staging_inc,
        op_kwargs={'date': business_dt,
    ...
    (
            ...
            >> get_report
            >> get_increment
            >> [upload_user_order_log_inc, upload_customer_research_inc, upload_user_activity_log_inc]
            >> update_d_item_table
            >> update_d_city_table
